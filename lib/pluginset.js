...
PluginSet.prototype._asyncWrapper = function(name) {
  var args = [].slice.call(arguments, 1),
      cbl = args.pop(),
      fns = {};
  ...
  this.plugins.forEach(function(i) {
    if (typeof i[name] === 'function')
      fns[i.name] = i[name].bind(this, args);
  });

  async.parallel(fns, cbl);
};
...
PluginSet.prototype._addMethod = function(name) {
  ...
  if (!this[name]) {
    ...
    this.async[name] = this._asyncWrapper.bind(this, name);
  }
};
...
PluginSet.prototype._executeConstructors = function() {
  this.plugins.forEach(function(plugin) {
    if (!plugin.used && plugin._constructor)
      plugin._constructor();
  });
};
